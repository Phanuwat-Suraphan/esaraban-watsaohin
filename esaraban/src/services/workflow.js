...
/**
 * บันทึกว่าผู้รับผิดชอบเปิดอ่านหนังสือฉบับนี้แล้ว — เรียกตอนเปิดหน้าหนังสือ
 *
 * ทำไมต้องมี: เดิมระบบรู้แค่ว่า "ยังไม่มีใครกดดำเนินการ" ซึ่งตอบไม่ได้ว่าเป็นเพราะเจ้าตัวยังไม่รู้ว่ามี
 * หนังสือ หรือรู้แล้วแต่ยังไม่ได้ทำ — สองอย่างนี้ต้องตามคนละแบบ (อย่างแรกคือแจ้งซ้ำให้รู้ตัว อย่างหลังคือ
 * ถามว่าติดอะไร) ธุรการที่ไล่ตามเรื่องจึงเดาเอาเองทุกครั้ง และครูที่เปิดอ่านแล้วแต่ยังทำไม่เสร็จก็โดนทวง
 * ซ้ำด้วยข้อความเดียวกับคนที่ยังไม่เคยเห็น
 *
 * บันทึกเฉพาะครั้งแรก (opened_at IS NULL) เพราะสิ่งที่ต้องการรู้คือ "รู้เรื่องนี้ตั้งแต่เมื่อไร" ไม่ใช่
 * เวลาที่เปิดล่าสุด — ถ้าเขียนทับทุกครั้งที่เปิด เวลาจะขยับเป็นปัจจุบันตลอดจนดูไม่ออกว่าค้างมานานแค่ไหน
 *
 * ผู้รักษาการแทนที่เปิดอ่านก็นับ เพราะเขาคือคนที่ดำเนินการแทนได้จริงในช่วงนั้น (ดู currentStepFor)
 * ส่วนธุรการ/ผอ./แอดมิน ที่เปิดดูเฉยๆ ไม่นับ — ไม่ใช่คนที่เรื่องค้างอยู่
 */
export function markStepOpened(documentId, userId) {
  const today = todayInBangkok();
  return db.prepare(`
    UPDATE workflow_steps SET opened_at = ?
    WHERE document_id = ? AND status = 'waiting' AND opened_at IS NULL
      AND (assignee_id = ? OR assignee_id IN (
        SELECT delegator_id FROM user_delegations
        WHERE delegate_id = ? AND cancelled_at IS NULL AND start_date <= ? AND end_date >= ?
      ))
  `).run(nowIso(), documentId, userId, userId, today, today).changes;
}
